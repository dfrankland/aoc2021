...
fn main_result() -> Result<(), ()> {
  ...
  for command_str in INPUT.lines() {
    let (command, unit_str) = command_str.split_once(" ").unwrap_or_else(|| unreachable!("Missing command or unit"));
  ...
  libc_println!("{} horizontal position and depth", horizontal_position * depth);
